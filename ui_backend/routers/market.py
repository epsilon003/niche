...
def get_latest_spectrogram(symbol: str = Query(...)):
    ...
    if result is None:
        raise HTTPException(
            status_code=404, detail=f"No spectrograms yet for {symbol.upper()}"
        )
    return result
...
def get_anomaly_scores(
    symbol: str = Query(...), limit: int = Query(default=200, ge=1, le=2000)
):
    return {
        "symbol": symbol.upper(),
        "scores": data_access.anomaly_history(symbol, limit=limit),
    }
