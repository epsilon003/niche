...
MIN_HISTORY_DAYS = (
    20  # ~1 trading month; below this a rank isn't statistically meaningful
)
...
def record_iv_sample(ticker: str, iv: float, as_of: date | None = None) -> None:
    ...
    existing = [
        row for row in existing if row["date"] != as_of.isoformat()
    ]  # one sample/day
...
def compute_iv_rank(
    ticker: str, current_iv: float, lookback_days: int = 252
) -> IvRankResult:
    ...
    rank = (
        50.0 if iv_max == iv_min else (current_iv - iv_min) / (iv_max - iv_min) * 100.0
    )
